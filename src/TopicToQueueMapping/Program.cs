...
namespace Tutorial
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine($"{nameof(TopicToQueueMapping)} Tutorial");
                ...
                var topicToQueueMapping = new TopicToQueueMapping();
                topicToQueueMapping.Run(host, vpnname, username, password);
            }
            catch(ArgumentException)
            {
                Console.WriteLine($"Usage:\n{nameof(TopicToQueueMapping)} [-h host] [-v vpn] [-u username] [-p password]");
            }
            catch(Exception ex)
            {
