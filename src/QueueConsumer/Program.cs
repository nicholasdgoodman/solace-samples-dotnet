...
namespace Tutorial
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine($"{nameof(QueueConsumer)} Tutorial");
                ...
                queueConsumer.Run(host, vpnname, username, password);
            }
            catch(ArgumentException)
            {
                Console.WriteLine($"Usage:\n{nameof(QueueConsumer)} [-h host] [-v vpn] [-u username] [-p password]");
            }
            catch(Exception ex)
            {
