...
namespace Tutorial
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine($"{nameof(ConfirmedPublish)} Tutorial");
                ...
                var queueProducer = new ConfirmedPublish();
                queueProducer.Run(host, vpnname, username, password);
            }
            catch(ArgumentException)
            {
                Console.WriteLine($"Usage:\n{nameof(ConfirmedPublish)} [-h host] [-v vpn] [-u username] [-p password]");
            }
            catch(Exception ex)
            {
