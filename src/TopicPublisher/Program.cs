...
namespace Tutorial
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine($"{nameof(TopicPublisher)} Tutorial");
                ...
                var topicSubscriber = new TopicPublisher();
                topicSubscriber.Run(host, vpnname, username, password);
            }
            catch(ArgumentException)
            {
                Console.WriteLine($"Usage:\n{nameof(TopicPublisher)} [-h host] [-v vpn] [-u username] [-p password]");
            }
            catch(Exception ex)
            {
