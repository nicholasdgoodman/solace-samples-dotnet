...
namespace Tutorial
{
    class Program
    {
        static void Main(string[] args)
        {
            try
            {
                Console.WriteLine($"{nameof(BasicRequestor)} Tutorial");
                ...
                var basicRequestor = new BasicRequestor();
                basicRequestor.Run(host, vpnname, username, password);
            }
            catch(ArgumentException)
            {
                Console.WriteLine($"Usage:\n{nameof(BasicRequestor)} [-h host] [-v vpn] [-u username] [-p password]");
            }
            catch(Exception ex)
            {
